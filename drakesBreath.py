...
if cc_value >= 1:
    ...
    return_string = (
        ...
        f'-desc "As an action, you can exhale a 30-foot cone of damaging breath or cause your drake to exhale it. Choose acid, cold, fire, lightning, or poison damage. Each creature in the cone must make a **DC {str(get_raw().spellbook.dc)}** Dexterity saving throw, taking {str(d)} damage on a failed save, or half as much (`{str(int(d.total/2))}`) damage on a successful one." '
        )
else:
    cc_use = 0
    return_string = (

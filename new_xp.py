...
if a:
    if len(a) > 2 or (len(a) > 1 and not a[1].lstrip('-').lstrip('+').isdigit()) or (len(a) > 0 and not a[0].lstrip('-').lstrip('+').isdigit()):
        rs += es
        return rs
    if len(a) > 0:
        cg = int(a[0])
...
rs += f' -f "Current Experience ({'+' if ct >= 0 else ''}{ct})|{cf}|inline"'
g = 0
if cg > 0:
    while cv < cp:
        if cv < 900 or cf < 900:
...
if ct != cg:
    if ct / cg < 0.4:
        ...
        g= ct / cg
if g != 0:
    rs += f' -f "Gold Rewards (not automatically added)|{round(g, 2)}|inline"'
return rs
</drac2>